...
import {AlimentCalc} from './alimentCalc';
...
export class AlimentService {
  ...
  addAliment(alimentData) {
    this.alimentLists.push(alimentData);

  }
  supprimerAliment(alimentId) {
    this.alimentLists.splice(alimentId, 1);
  }
  supprimerAlimentCalc(alimentCalcId) {
    this.cgAlimentSup = this.alimentListsCalcul[alimentCalcId].cg;
    this.alimentListsCalcul.splice(alimentCalcId, 1);
  }
  calculAliment(portionData) {
    let portion = portionData.portion;
    let name = portionData.alimentPortion;
    let ig;
    let carbs;
  
    
    for (let i = 0; i < this.alimentLists.length; i++) {

      if (this.alimentLists[i].name == name) {
        ig = this.alimentLists[i].ig;
        carbs = this.alimentLists[i].carbs;
      }
    }
    this.cgAliment = (ig * (carbs * portion / 100)) / 100;
    let cgbis=this.cgAliment
  
    let alimentAjcalc:AlimentCalc ={name: name, ig: ig, carbs: carbs, cg: cgbis};
  
    this.alimentListsCalcul.push(alimentAjcalc);
  }
}
